...
#     AGENTS  : space or comma separated subset of: claude gemini copilot cursor qwen opencode windsurf codex kilocode auggie roo droid (default: all)
...
build_variant() {
  ...
  if [[ -d templates ]]; then
    ...
    while IFS= read -r -d '' file; do
      rsync -R "$file" "$SPEC_DIR"/
    done < <(find templates -type f -not -path "templates/commands/*" -print0)
  ...
  case $agent in
    ...
    droid)
      mkdir -p "$base_dir/.factory/commands" "$base_dir/.droid/commands"
      generate_commands droid md "\$ARGUMENTS" "$base_dir/.factory/commands" "$script"
      cp -R "$base_dir/.factory/commands/." "$base_dir/.droid/commands/" 2>/dev/null || true
      mkdir -p "$base_dir/.factory/rules" "$base_dir/.droid/rules" ;;
  esac
...
ls -1 "$GENRELEASES_DIR"/dev-flow-template-*-"${NEW_VERSION}".zip
